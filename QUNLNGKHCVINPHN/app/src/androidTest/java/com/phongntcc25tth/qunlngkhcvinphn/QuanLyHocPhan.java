    ...
    public class qtv_QuanLyHocPhan extends AppCompatActivity {
        ...
        protected void onCreate(Bundle b) {
            ...
            btnThem.setOnClickListener(v -> {
                ...
                if (ma.isEmpty() || ten.isEmpty()) { Toast.makeText(this, " Nhập Mã & Tên HP!", Toast.LENGTH_SHORT).show(); return; }
                long kq = db.themHP(ma, ten, Integer.parseInt(edtTC.getText().toString().trim()),
                        edtNganh.getText().toString().trim(), Integer.parseInt(edtHK.getText().toString().trim()));
                Toast.makeText(this, kq > 0 ? " Thêm thành công!" : " Mã HP đã tồn tại!", Toast.LENGTH_SHORT).show();
            });

            btnSua.setOnClickListener(v -> {
                String ma = edtMaHP.getText().toString().trim();
                int kq = db.suaHP(ma, edtTenHP.getText().toString().trim(),
